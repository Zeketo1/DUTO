...
const Header = ({ setShowLogin }) => {
    useEffect(() => {
        onAuthStateChanged(auth, async (user) => {
            if (user) {
                document.getElementById("googleProfileImage").src =
                    `${user.photoURL}`;
            } else {
                document.getElementById("googleProfileImage").src = {
                    depotLogo,
    ...
    const navLeft = [
        ...
        {
            icon: <SearchSharp className="rotate-90 mr-5" />,
            text: (
                <MenuSharp
                    className="cursor-pointer"
                    onClick={() =>
                        setBar(
                            "right-0 bg-[#000] min-h-[100vh] w-[37.8rem] top-0 fixed z-[9999] transit flex flex-col justify-between pb-10"
                        )
                    }
                />
            ),
    ...
    return (
        <>
            <div className=" flex justify-around items-center bg-black text-white sticky top-0 h-24 text-[12px] font-bold z-[999] backdrop-opacity-5 backdrop-invert bg-black/95 px-">
                ...
                <h1 className="font-bold text-[25px] tracking-[0.5rem]">
                    DUTO
                </h1>
